...
def run_inference(opt, source, target, RetinaFace,
    ...
    try:
        ...
        for annotation in faces:
            ...
            cv2.imshow("img",face_swap)
            cv2.waitKey()
            cv2.destroyAllWindows()
            face_swap=np.clip(face_swap * 255, 0, 255).astype('uint8')
            cv2.imwrite("face_swap.jpg", cv2.cvtColor(face_swap, cv2.COLOR_BGR2RGB))
...
def video_swap(opt, face, input_video, RetinaFace, ArcFace, FaceDancer, out_video_filename):
    ...
    print("out_video_filename",out_video_filename)
    print(type(out_video_filename))
